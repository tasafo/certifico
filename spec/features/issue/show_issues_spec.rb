...
describe 'Show issues', js: true do
  ...
  let!(:subscriber)   { create(:subscriber, user: user2, certificate: certificate, profile: profile) }
  ...
  context 'with same dates' do
    before do
      ...
      click_link 'Emissões'
      visit issue_path(subscriber)
    end

    it 'redirects to the emission page' do
      expect(current_path).to eql(issue_path(subscriber))
    end
  end
